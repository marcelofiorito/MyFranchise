...
**MVP:** Five integrated modules — Network Dashboard, Governance & Compliance, Franchisee Portal, Onboarding, and Stockout Rupture Management — delivered on SAP CAP, Fiori Elements, and HANA Cloud, with generative artificial intelligence for proactive recommendations to the franchisee and stockout analysis via Joule.
...
**Representative:** Franchisee, owner of a Standard unit in the fashion/lifestyle segment
...
- Identify stockout risks and act proactively
...
**Fiori Floorplan:** Overview Page (OVP) with 8 cards

#### RF-FRAN-01 — Franchisee Dashboard (OVP)
- Upon logging in, the franchisee must see an Overview Page with:
  - **Card 1:** Revenue — period revenue, MoM variation, target vs. actual
  - **Card 2:** Health Score — unit score vs. cluster average
  - **Card 3:** Category Margins — average ticket and margin by product category
  - **Card 4:** Campaign Activation — active campaigns and adherence status
  - **Card 5:** Pending Actions — top 5 most urgent actions across all modules
  - **Card 6:** AI Recommendations — top 3 recommendations generated by AI Core
  - **Card 7:** Stock Alerts — current stockout and stockout risk by Color×Size grid
  - **Card 8:** Network Position — anonymous ranking of the franchisee within the cluster
...
### 7.5 Module: Stockout Rupture Management

**Objective:** Identify stockout risks by Color×Size grid before they impact sales, and guide the franchisee with concrete replenishment options — including substitutes — via Joule.

**Fiori Floorplan:** Analytical List Page (ALP) + Object Page; MCP tools in Joule

#### RF-RUP-01 — Color×Size Grid
- All stock analysis must be performed with a Color×Size grid breakdown
- Each grid item corresponds to a unique SKU with fields: `skuId`, `produtoId`, `cor` (color), `tamanho` (size), `saldoAtual` (current stock)
- The grid must be viewable by product and by store, grouped by color and sorted by size

#### RF-RUP-02 — 14-Day Demand Forecast and Projected Balance
- For each grid item, the system must calculate:
  - `previsaoDemanda14d`: units expected to sell in the next 14 days (based on historical average)
  - `saldoProjetado14d`: `saldoAtual` − `previsaoDemanda14d`
  - `rupturaEm`: number of days until projected stockout (null if projected balance ≥ 0)
- Items with `saldoProjetado14d < 0` are classified as **RUPTURA** (stockout)
- Items with `rupturaEm` between 1 and 14 days are classified as **RISCO** (at risk)

#### RF-RUP-03 — Substitute Product Mapping
- The system must maintain a `ProdutoSubstituto` entity with the following fields:
  - `skuOrigem` (source SKU), `skuSubstituto` (substitute SKU), `similaridade` (0–100%), `tipoSimilaridade` (Color/Style/Model)
- The franchisor manages substitutes via the administrative catalog
- Substitute searches must return only items with available stock in the same store

#### RF-RUP-04 — "If X → Offer Y" Guide
- Joule must expose the tool `get_substitutos(skuId, lojaId)` which returns:
  - A list of available substitutes with `similaridade`, `tipoSimilaridade`, and `saldoAtual`
  - A sales approach recommendation: offer as a direct alternative or complement
- The tool must be actionable in natural language: "What substitutes do I have for the blue sneaker in size 40?"

#### RF-RUP-05 — Grid Analysis via Joule
- Joule must expose the tool `get_grade_ruptura(produtoId, lojaId)` which returns:
  - A full Color×Size matrix with `saldoAtual`, `previsaoDemanda14d`, `saldoProjetado14d`, `rupturaEm`, and status (OK / RISCO / RUPTURA)
  - Summary: total items, items in RUPTURA, items at RISCO, items OK
- The tool must be actionable in natural language: "How is the black jacket stock by size?"

#### RF-RUP-06 — Three Replenishment Strategies
- When stockout or at-risk items are identified, the system must present the franchisee with three options:
  1. **Full Replenishment:** request full replenishment of critical items from the franchisor
  2. **Substitutes Only:** focus on selling available substitute products
  3. **Combined Strategy:** replenish the highest-impact items and use substitutes for the rest
- Each option must include the estimated financial impact (`valorImpactoStockout`) to support the decision

#### RF-RUP-07 — Financial Impact per Grid Item
- For each stockout grid item, the system must calculate `valorImpactoStockout`:
  - Formula: `previsaoDemanda14d × precoMedio`
  - Displayed in R$ in the item detail and in the analysis summary
- `valorImpactoStockout` must be used to prioritize the replenishment order
...
| SAP Analytics Cloud (SAC) | SAC Story iFrame via HANA Live Connection in Work Zone (sac-overview) |
...
| Sprint 3 | 12/08 – 18/08 | OVP Franchisee Portal (8 cards) + AI Core integration + LROP Onboarding |
| Sprint 4 | 19/08 – 25/08 | Color×Size Grid + Substitutes entity + Joule MCP tools + Deploy BTP CF + Work Zone + polish + rehearsals (2,022 stock items, 117 RUPTURA) |
...
*Document updated in August 2026. Next review planned after the Dragons' Den demo (26/08/2026).*
